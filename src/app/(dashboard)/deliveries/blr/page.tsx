...
export default function BLRDeliveriesPage() {
  ...
  const [deliveries, setDeliveries] = useState<BLRDelivery[]>([]);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      // Fetch non-outstation deliveries (BLR = local)
      const params = new URLSearchParams({ outstation: "false", limit: "100" });
      if (statusFilter !== "ALL") params.set("status", statusFilter);
      if (debouncedSearch) params.set("search", debouncedSearch);
      const res = await fetch(`/api/deliveries?${params}`).then(r => r.json());
      if (res.success) {
        // Filter out WALK_OUT on client side (they have their own page)
        setDeliveries(res.data.filter((d: BLRDelivery) => d.status !== "WALK_OUT" && d.status !== "PREBOOKED"));
      }
    } catch { /* ignore */ }
    setLoading(false);
  ...
  const FILTERS = ["ALL", "PENDING", "SCHEDULED", "OUT_FOR_DELIVERY", "DELIVERED"];
  ...
  return (
    <div className="pb-20">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h1 className="text-lg font-bold text-slate-900">Bangalore Deliveries</h1>
      ...
      <div className="flex gap-1.5 overflow-x-auto scrollbar-hide mb-3 pb-1">
        {FILTERS.map((f) => (
          <button key={f} onClick={() => setStatusFilter(f)}
            ...
            {f === "ALL" ? "All" : f === "OUT_FOR_DELIVERY" ? "Out for Delivery" : f.charAt(0) + f.slice(1).toLowerCase()}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-slate-500">No Bangalore deliveries</p>
        </div>
      ) : (
                ...
                <CardContent className="p-3.5">
                  ...
                  <div className="flex items-center justify-between mt-1.5">
                    <div className="flex items-center gap-2 text-[10px] text-slate-500">
                      ...
                      {d.scheduledDate && <span>📅 {new Date(d.scheduledDate).toLocaleDateString("en-IN")}</span>}
                    </div>
                    {d.customerPhone && (
